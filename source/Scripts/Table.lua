...
function manager_table.ForEvery(iterator: number, table_to_search: Array)
	local reconstructed = {}
	local remainder = {}
	local value = {}
	...
	if (iterator > #table_to_search) then
		...
		for i = 1, #table_to_search do
			...
			if (manager_math.IsWhole(i, iterator)) then
...
function manager_table.ToArray(table_to_parse: table)
	local keys, values = {}, {}

	for k, v in pairs(table_to_parse) do
		table.insert(keys, k)
